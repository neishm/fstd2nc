...
class Masks (BufferBase):
  ...
  def __init__ (self, *args, **kwargs):
    ...
    self._ignore_atts = ('mask_record','mask_length') + self._ignore_atts
  ...
  def _decode (self, data, mask=None, **kwargs):
    import numpy as np
    prm = self._decode_headers(data[:72])
    prm = dict((k,v[0]) for k,v in prm.items())
    # Get first field.
    field1 = super(Masks,self)._decode(data, **kwargs)
    ...
    if mask is None:
      return field1
    field2 = super(Masks,self)._decode(mask, **kwargs)
